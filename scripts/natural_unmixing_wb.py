...
def main():
    ...
    for i in range(1, n+1):
        file_path = f"{path}/cls_nu/cls_data_wb_nu_{i}_l1max_{lmax_partial}_l2max_{lmax_mask}.fits"
        if os.path.exists(file_path) and recompute=="False":
            nu_cls = heracles.read(file_path)
        else:
            # Load cls
            wcls_path = f"{path}/cls/wcls_data_wb_{i}_lmax_{lmax_partial}.fits"
            if os.path.exists(wcls_path):
                wcls = heracles.read(wcls_path)
            else:
                print(f"computing wcl {i}")
                data_cls = heracles.read(f"{path}/cls/cls_data_wb_{i}_lmax_{lmax_full}.fits")
    ...
    heracles.write(path+f"/covs/cov_nu_cqs_wb_l1max_{lmax_full}_l2max_{lmax_mask}.fits", nu_cqs_cov)
